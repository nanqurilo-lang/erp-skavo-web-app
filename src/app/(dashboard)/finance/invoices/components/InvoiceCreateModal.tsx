...
//       if (inv.project?.projectName) {
//         set.add(inv.project.projectName);
//       }
//     });
//     return ["Select Project", ...Array.from(set)];
//   }, [invoices]);
...
//     invoices.forEach((inv) => {
//       if (inv.client?.name) {
//         set.add(inv.client.name);
//       }
//     });
//     return ["Select Client", ...Array.from(set)];
//   }, [invoices]);

//   // 🔹 Helpers (signature SAME)
//   const getProjectBudget = (projectName) => {
//     const f = invoices.find(
//       (i) => i.project?.projectName === projectName
//     );
//     return f?.project?.budget || 0;
//   };
...
//   amountInWords: form.amountInWords?.trim() || "",
...
//   className="border rounded w-full h-10 px-2"
//   value={
//     invoices.find(i => i.client?.clientId === form.clientId)?.client?.name || ""
//   }
//   onChange={(e) => {
//     const name = e.target.value;
//     const found = invoices.find(
...
export default function InvoiceCreateModal({
  ...
  const projectList = useMemo(() => {
    const set = new Set();
    invoices.forEach(
      (inv) => inv.project?.projectName && set.add(inv.project.projectName)
    );
    return ["Select Project", ...Array.from(set)];
  }, [invoices]);
