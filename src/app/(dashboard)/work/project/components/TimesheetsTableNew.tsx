...
// ===== Project / Task dependent dropdown states =====
...
const [projectsLoading, setProjectsLoading] = useState(false);
  ...
  try {
    const url = buildUrl("/projects/AllProject");
...
const fetchProjectTasks = async (projectIdRaw: string | number) => {
  if (!projectIdRaw) {
    setProjectTasks([]);
  ...
  try {
    const url = buildUrl(`/projects/${projectIdRaw}/tasks`);
    const res = await doFetch(url, {
      method: "GET",
      headers: headersWithAuth({ "Content-Type": "application/json" }),
    });
            ...
            <div className="p-6">
              <div className="bg-white rounded-md border p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm text-gray-700 mb-2">Project *</label>
                    {/* <input value={form.projectId} onChange={(e) => setForm((s) => 
                      ({ ...s, projectId: e.target.value }))} 
                       className="w-full border rounded px-3 py-2" placeholder="Project ID" type="text" /> */}
...
>
  ...
  {projectOptions.map((p) => (
    <option key={p.id} value={String(p.id)}>
      {p.name || p.shortCode || `Project ${p.id}`}
    </option>
  ))}
</select>
                  ...
                  <div>
                    <label className="block text-sm text-gray-700 mb-2">Task *</label>
                    {/* <input value={form.taskId} onChange={(e) => setForm((s) =>
                       ({ ...s, taskId: e.target.value }))} 
                       className="w-full border rounded px-3 py-2" placeholder="Task ID" type="text" /> */}
...
<select
  ...
  onChange={(e) => {
    ...
    const task = projectTasks.find((t: any) => String(t.id) === val);
...
>
  ...
  {projectTasks.map((t: any) => (
    <option key={t.id} value={String(t.id)}>
      {t.title ? `Task ${t.id} - ${t.title}` : `Task ${t.id}`}
    </option>
  ))}
</select>
                  ...
                  <div>
                    <label className="block text-sm text-gray-700 mb-2">Employee *</label>
                    {/* <select value={form.employeeId} onChange={(e) => setForm((s) =>
                       ({ ...s, employeeId: e.target.value }))} className="w-full border rounded px-3 py-2">
                      <option value="">--</option>
                      {employeeList.map((e) => (
                        <option key={e.employeeId} value={e.employeeId}>
                          {e.name ? `${e.name} (${e.employeeId})` : e.employeeId}
